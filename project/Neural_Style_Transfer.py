...
def compute_loss(model, loss_weights, init_image, gram_style_features,
                 content_features, num_content_layers, num_style_layers):
...
def run_style_transfer(content_path,
    ...
    norm_means = np.array([103.939, 116.776, 123.68])
    min_vals = -norm_means
...
def save_result_image(img, img_name):
    try:
        img_dir = './results'
        img.save('{}/{}'.format(img_dir, img_name))
        if not os.path.exists(img_dir):
            os.makedirs(img_dir)
        print("Sucessfully saved the output image!")
    except IOError:
...
def main():
    ...
    while True:
        ...
        if cmd == 'y' or cmd == 'Y':
            output_image_name = input("File name for out file: ")
